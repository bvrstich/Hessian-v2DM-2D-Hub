...
ostream &operator<<(ostream &output,const EIG &eig_p){
   ...
   output << eig_p.gvI() << std::endl;

#ifdef __Q_CON
   output << eig_p.gvQ() << std::endl;
#endif

#ifdef __G_CON
   output << eig_p.gvG() << std::endl;
#endif

#ifdef __T1_CON
   output << eig_p.gvT1() << std::endl;
#endif

#ifdef __T2_CON
   output << eig_p.gvT2() << std::endl;
#endif
...
double EIG::center_dev() const{
   ...
   double sum = vI->sum() + vQ->sum();

   double log_product = vI->log_product() + vQ->log_product();
